...
def backup_sqlite_data():
    ...
    for model_name in models:
        try:
            ...
            count = len(backup_data[model_name])
            total_objects += count
            ...
            try:
                ...
                for obj_data in objects_data:
                    try:
                        ...
                        print(f"⚠️ Erreur lors de la restauration d'un objet {model_name}: {e}")
                        continue
                
                print(f"✅ {model_name}: {restored_count} objets restaurés")
...
def main():
    ...
    try:
        ...
        total_objects = sum(len(data) for data in backup_data.values())
        if total_objects == 0:
