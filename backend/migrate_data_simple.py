...
def main():
    ...
    if not run_command("python manage.py dumpdata --exclude auth.permission --exclude contenttypes --indent 2 > sqlite_data_backup.json", sqlite_env):
        print("❌ Échec de la sauvegarde des données SQLite")
        return
    
    # Vérifier si le fichier de sauvegarde contient des données
    backup_file = Path("sqlite_data_backup.json")
    ...
    if not run_command("python manage.py loaddata sqlite_data_backup.json", postgresql_env):
